...
export default class UtilsStr {
  ...
  // #####
  // region Regexes
  static regexHashTagList = /(^|\W)#([a-zA-Zа-яА-Я_]+[0-9\w-]*)/gim;
  // endregion Regexes
  // #####
  static hashtag(str) {
  ...
  static transformEmail(str) {
    // Check if input is a valid email format (xxx@yyy.ZZZ)
    // Pattern: at least one character before @, at least one character between @ and ., at least 2 characters after .
    ...
    // Return the username in uppercase (XXX format)
    return `@: ${username}`;
  }
}
